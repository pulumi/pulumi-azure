...
func GetLB(ctx *pulumi.Context, args *GetLBArgs, opts ...pulumi.InvokeOption) (*GetLBResult, error) {
	...
	if optsErr != nil {
		return &GetLBResult{}, optsErr
	}
	if len(invokeOpts.DependsOn) > 0 {
		return &GetLBResult{}, errors.New("DependsOn is not supported for direct form invoke GetLB, use GetLBOutput instead")
	}
	if len(invokeOpts.DependsOnInputs) > 0 {
		return &GetLBResult{}, errors.New("DependsOnInputs is not supported for direct form invoke GetLB, use GetLBOutput instead")
	}
	var rv GetLBResult
...
func GetLBOutput(ctx *pulumi.Context, args GetLBOutputArgs, opts ...pulumi.InvokeOption) GetLBResultOutput {
	return pulumi.ToOutputWithContext(ctx.Context(), args).
		ApplyT(func(v interface{}) (GetLBResultOutput, error) {
			...
			opts = internal.PkgInvokeDefaultOpts(opts)
			var rv GetLBResult
			secret, deps, err := ctx.InvokePackageRawWithDeps("azure:lb/getLB:getLB", args, &rv, "", opts...)
			if err != nil {
				return GetLBResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(GetLBResultOutput)
			output = pulumi.OutputWithDependencies(ctx.Context(), output, deps...).(GetLBResultOutput)
			if secret {
