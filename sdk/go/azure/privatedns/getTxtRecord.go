...
func LookupTxtRecord(ctx *pulumi.Context, args *LookupTxtRecordArgs, opts ...pulumi.InvokeOption) (*LookupTxtRecordResult, error) {
	...
	if optsErr != nil {
		return &LookupTxtRecordResult{}, optsErr
	}
	if len(invokeOpts.DependsOn) > 0 {
		return &LookupTxtRecordResult{}, errors.New("DependsOn is not supported for direct form invoke LookupTxtRecord, use LookupTxtRecordOutput instead")
	}
	if len(invokeOpts.DependsOnInputs) > 0 {
		return &LookupTxtRecordResult{}, errors.New("DependsOnInputs is not supported for direct form invoke LookupTxtRecord, use LookupTxtRecordOutput instead")
	}
...
func LookupTxtRecordOutput(ctx *pulumi.Context, args LookupTxtRecordOutputArgs, opts ...pulumi.InvokeOption) LookupTxtRecordResultOutput {
	return pulumi.ToOutputWithContext(ctx.Context(), args).
		ApplyT(func(v interface{}) (LookupTxtRecordResultOutput, error) {
			...
			opts = internal.PkgInvokeDefaultOpts(opts)
			var rv LookupTxtRecordResult
			secret, deps, err := ctx.InvokePackageRawWithDeps("azure:privatedns/getTxtRecord:getTxtRecord", args, &rv, "", opts...)
			if err != nil {
				return LookupTxtRecordResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(LookupTxtRecordResultOutput)
			output = pulumi.OutputWithDependencies(ctx.Context(), output, deps...).(LookupTxtRecordResultOutput)
			if secret {
