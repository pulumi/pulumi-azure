...
class SparkCluster(pulumi.CustomResource):
    ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        ...
        if resource_args is not None:
            ...
            def _setter(key, value):
                kwargs[key] = value
            SparkClusterArgs._configure(_setter, **kwargs)
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if component_version is not None and not isinstance(component_version, SparkClusterComponentVersionArgs):
                component_version = component_version or {}
                def _setter(key, value):
                    component_version[key] = value
                SparkClusterComponentVersionArgs._configure(_setter, **component_version)
            ...
            if compute_isolation is not None and not isinstance(compute_isolation, SparkClusterComputeIsolationArgs):
                compute_isolation = compute_isolation or {}
                def _setter(key, value):
                    compute_isolation[key] = value
                SparkClusterComputeIsolationArgs._configure(_setter, **compute_isolation)
            ...
            if extension is not None and not isinstance(extension, SparkClusterExtensionArgs):
                extension = extension or {}
                def _setter(key, value):
                    extension[key] = value
                SparkClusterExtensionArgs._configure(_setter, **extension)
            __props__.__dict__["extension"] = extension
            if gateway is not None and not isinstance(gateway, SparkClusterGatewayArgs):
                gateway = gateway or {}
                def _setter(key, value):
                    gateway[key] = value
                SparkClusterGatewayArgs._configure(_setter, **gateway)
            if gateway is None and not opts.urn:
                raise TypeError("Missing required property 'gateway'")
            __props__.__dict__["gateway"] = gateway
            __props__.__dict__["location"] = location
            if metastores is not None and not isinstance(metastores, SparkClusterMetastoresArgs):
                metastores = metastores or {}
                def _setter(key, value):
                    metastores[key] = value
                SparkClusterMetastoresArgs._configure(_setter, **metastores)
            __props__.__dict__["metastores"] = metastores
            if monitor is not None and not isinstance(monitor, SparkClusterMonitorArgs):
                monitor = monitor or {}
                def _setter(key, value):
                    monitor[key] = value
                SparkClusterMonitorArgs._configure(_setter, **monitor)
            __props__.__dict__["monitor"] = monitor
            __props__.__dict__["name"] = name
            if network is not None and not isinstance(network, SparkClusterNetworkArgs):
                network = network or {}
                def _setter(key, value):
                    network[key] = value
                SparkClusterNetworkArgs._configure(_setter, **network)
            __props__.__dict__["network"] = network
            if resource_group_name is None and not opts.urn:
                raise TypeError("Missing required property 'resource_group_name'")
            __props__.__dict__["resource_group_name"] = resource_group_name
            if roles is not None and not isinstance(roles, SparkClusterRolesArgs):
                roles = roles or {}
                def _setter(key, value):
                    roles[key] = value
                SparkClusterRolesArgs._configure(_setter, **roles)
            if roles is None and not opts.urn:
                raise TypeError("Missing required property 'roles'")
            __props__.__dict__["roles"] = roles
            if security_profile is not None and not isinstance(security_profile, SparkClusterSecurityProfileArgs):
                security_profile = security_profile or {}
                def _setter(key, value):
                    security_profile[key] = value
                SparkClusterSecurityProfileArgs._configure(_setter, **security_profile)
            __props__.__dict__["security_profile"] = security_profile
            if storage_account_gen2 is not None and not isinstance(storage_account_gen2, SparkClusterStorageAccountGen2Args):
                storage_account_gen2 = storage_account_gen2 or {}
                def _setter(key, value):
                    storage_account_gen2[key] = value
                SparkClusterStorageAccountGen2Args._configure(_setter, **storage_account_gen2)
