// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.azure.datafactory.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class LinkedServiceOdbcBasicAuthentication {
    /**
     * @return The password associated with the username, which can be used to authenticate to the ODBC endpoint.
     * 
     */
    private String password;
    /**
     * @return The username which can be used to authenticate to the ODBC endpoint.
     * 
     */
    private String username;

    private LinkedServiceOdbcBasicAuthentication() {}
    /**
     * @return The password associated with the username, which can be used to authenticate to the ODBC endpoint.
     * 
     */
    public String password() {
        return this.password;
    }
    /**
     * @return The username which can be used to authenticate to the ODBC endpoint.
     * 
     */
    public String username() {
        return this.username;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(LinkedServiceOdbcBasicAuthentication defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private String password;
        private String username;
        public Builder() {}
        public Builder(LinkedServiceOdbcBasicAuthentication defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.password = defaults.password;
    	      this.username = defaults.username;
        }

        @CustomType.Setter
        public Builder password(String password) {
            if (password == null) {
              throw new MissingRequiredPropertyException("LinkedServiceOdbcBasicAuthentication", "password");
            }
            this.password = password;
            return this;
        }
        @CustomType.Setter
        public Builder username(String username) {
            if (username == null) {
              throw new MissingRequiredPropertyException("LinkedServiceOdbcBasicAuthentication", "username");
            }
            this.username = username;
            return this;
        }
        public LinkedServiceOdbcBasicAuthentication build() {
            final var _resultValue = new LinkedServiceOdbcBasicAuthentication();
            _resultValue.password = password;
            _resultValue.username = username;
            return _resultValue;
        }
    }
}
