// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.azure.automanage.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs extends com.pulumi.resources.ResourceArgs {

    public static final ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs Empty = new ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs();

    /**
     * The count of the retention duration of the backup policy. Valid value inside `daily_schedule` is `7` to `9999` and inside `weekly_schedule` is `1` to `5163`.
     * 
     */
    @Import(name="count")
    private @Nullable Output<Integer> count;

    /**
     * @return The count of the retention duration of the backup policy. Valid value inside `daily_schedule` is `7` to `9999` and inside `weekly_schedule` is `1` to `5163`.
     * 
     */
    public Optional<Output<Integer>> count() {
        return Optional.ofNullable(this.count);
    }

    /**
     * The duration type of the retention duration of the backup policy. Valid value inside `daily_schedule` is `Days` and inside `weekly_schedule` is `Weeks`. Defaults to `Days`.
     * 
     */
    @Import(name="durationType")
    private @Nullable Output<String> durationType;

    /**
     * @return The duration type of the retention duration of the backup policy. Valid value inside `daily_schedule` is `Days` and inside `weekly_schedule` is `Weeks`. Defaults to `Days`.
     * 
     */
    public Optional<Output<String>> durationType() {
        return Optional.ofNullable(this.durationType);
    }

    private ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs() {}

    private ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs(ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs $) {
        this.count = $.count;
        this.durationType = $.durationType;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs $;

        public Builder() {
            $ = new ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs();
        }

        public Builder(ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs defaults) {
            $ = new ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param count The count of the retention duration of the backup policy. Valid value inside `daily_schedule` is `7` to `9999` and inside `weekly_schedule` is `1` to `5163`.
         * 
         * @return builder
         * 
         */
        public Builder count(@Nullable Output<Integer> count) {
            $.count = count;
            return this;
        }

        /**
         * @param count The count of the retention duration of the backup policy. Valid value inside `daily_schedule` is `7` to `9999` and inside `weekly_schedule` is `1` to `5163`.
         * 
         * @return builder
         * 
         */
        public Builder count(Integer count) {
            return count(Output.of(count));
        }

        /**
         * @param durationType The duration type of the retention duration of the backup policy. Valid value inside `daily_schedule` is `Days` and inside `weekly_schedule` is `Weeks`. Defaults to `Days`.
         * 
         * @return builder
         * 
         */
        public Builder durationType(@Nullable Output<String> durationType) {
            $.durationType = durationType;
            return this;
        }

        /**
         * @param durationType The duration type of the retention duration of the backup policy. Valid value inside `daily_schedule` is `Days` and inside `weekly_schedule` is `Weeks`. Defaults to `Days`.
         * 
         * @return builder
         * 
         */
        public Builder durationType(String durationType) {
            return durationType(Output.of(durationType));
        }

        public ConfigurationBackupRetentionPolicyWeeklyScheduleRetentionDurationArgs build() {
            return $;
        }
    }

}
