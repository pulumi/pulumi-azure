// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.azure.automation.inputs;

import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;


public final class GetRunbookPlainArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetRunbookPlainArgs Empty = new GetRunbookPlainArgs();

    /**
     * The name of the Automation Account the runbook belongs to.
     * 
     */
    @Import(name="automationAccountName", required=true)
    private String automationAccountName;

    /**
     * @return The name of the Automation Account the runbook belongs to.
     * 
     */
    public String automationAccountName() {
        return this.automationAccountName;
    }

    /**
     * The name of the Automation Runbook.
     * 
     */
    @Import(name="name", required=true)
    private String name;

    /**
     * @return The name of the Automation Runbook.
     * 
     */
    public String name() {
        return this.name;
    }

    /**
     * The name of the Resource Group where the Automation exists.
     * 
     */
    @Import(name="resourceGroupName", required=true)
    private String resourceGroupName;

    /**
     * @return The name of the Resource Group where the Automation exists.
     * 
     */
    public String resourceGroupName() {
        return this.resourceGroupName;
    }

    private GetRunbookPlainArgs() {}

    private GetRunbookPlainArgs(GetRunbookPlainArgs $) {
        this.automationAccountName = $.automationAccountName;
        this.name = $.name;
        this.resourceGroupName = $.resourceGroupName;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetRunbookPlainArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetRunbookPlainArgs $;

        public Builder() {
            $ = new GetRunbookPlainArgs();
        }

        public Builder(GetRunbookPlainArgs defaults) {
            $ = new GetRunbookPlainArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param automationAccountName The name of the Automation Account the runbook belongs to.
         * 
         * @return builder
         * 
         */
        public Builder automationAccountName(String automationAccountName) {
            $.automationAccountName = automationAccountName;
            return this;
        }

        /**
         * @param name The name of the Automation Runbook.
         * 
         * @return builder
         * 
         */
        public Builder name(String name) {
            $.name = name;
            return this;
        }

        /**
         * @param resourceGroupName The name of the Resource Group where the Automation exists.
         * 
         * @return builder
         * 
         */
        public Builder resourceGroupName(String resourceGroupName) {
            $.resourceGroupName = resourceGroupName;
            return this;
        }

        public GetRunbookPlainArgs build() {
            if ($.automationAccountName == null) {
                throw new MissingRequiredPropertyException("GetRunbookPlainArgs", "automationAccountName");
            }
            if ($.name == null) {
                throw new MissingRequiredPropertyException("GetRunbookPlainArgs", "name");
            }
            if ($.resourceGroupName == null) {
                throw new MissingRequiredPropertyException("GetRunbookPlainArgs", "resourceGroupName");
            }
            return $;
        }
    }

}
