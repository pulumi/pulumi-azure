...
PLUGIN_PACKAGE_NAME="pulumi-provider-azurerm-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   ...
   -o "${WORK_PATH}/pulumi-provider-azurerm${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-provider-azure"
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-provider-azure" \
