...
PLUGIN_PACKAGE_NAME="pulumi-resource-azure-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
(cd "${ROOT}/provider" && go build \
   -ldflags "-X github.com/pulumi/pulumi-azure/provider/v2/pkg/version.Version=${VERSION}" \
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-resource-azure" \
